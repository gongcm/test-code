...
#endif
#define GREEN_LIGHT_GPIO (5*8 + 0)
#define RED_LIGHT_GPIO (5*8 + 4)
#define BLUE_LIGHT_GPIO (5*8 + 3)
	s_pGpioFunc->pfnGpioDirSetBit(GREEN_LIGHT_GPIO, 0);
	s_pGpioFunc->pfnGpioDirSetBit(RED_LIGHT_GPIO, 0);
	s_pGpioFunc->pfnGpioDirSetBit(BLUE_LIGHT_GPIO, 0);
	if(bSuspend)
	{
		s_pGpioFunc->pfnGpioWriteBit(GREEN_LIGHT_GPIO, 0);
		s_pGpioFunc->pfnGpioWriteBit(RED_LIGHT_GPIO, 1);
		s_pGpioFunc->pfnGpioWriteBit(BLUE_LIGHT_GPIO, 0);
		HI_INFO_PM("Suspend Red Light\n");
	}
	else
	{
		s_pGpioFunc->pfnGpioWriteBit(GREEN_LIGHT_GPIO, 1);
		s_pGpioFunc->pfnGpioWriteBit(RED_LIGHT_GPIO, 0);
		s_pGpioFunc->pfnGpioWriteBit(BLUE_LIGHT_GPIO, 0);
		HI_INFO_PM("Suspend Green Light\n");
	}
	HI_INFO_PM("Light set ok  just Green or Red ,No blue\n");
    return;
}
#endif
...
static HI_S32 c51_suspend (PM_BASEDEV_S *pdev, pm_message_t state)
{
   ...
   SetGpioLightSuspend(HI_TRUE);
	...
	HI_INFO_PM("c51 start work ok! add by gongcm\n");
