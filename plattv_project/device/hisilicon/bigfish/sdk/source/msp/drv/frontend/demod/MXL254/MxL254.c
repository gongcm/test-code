...

{
  ...
  if (annexType !=  MXL_HRCLS_ANNEX_OOB)
  {
    ...
    else
    {
      ...
      mpegXptParams.clkFreq = MXL_HRCLS_MPEG_CLK_28_16MHz; 
   ...
   ************************************************************************************/
	...
	// xptSupported = MXL_TRUE;
	  ...
	  if (xptSupported == MXL_TRUE)
	  {
		...
		/* modified by chenzw, for parallel ts output, org_value: MXL_HRCLS_XPT_MODE_NO_MUX_4*/
	    status = MxLWare_HRCLS_API_CfgXpt(HRCLS_DEVICE_ID, MXL_HRCLS_XPT_MODE_PARALLEL);
        if (status != MXL_SUCCESS)
    	  {
...
#if 0 //Must be disabled in parallel TS mode
