...
{
    ALOGE("usage: %s [-m 0|1] [-u 0|1] [-U num] [-r 0|1] [-R num] [-T readtimeout] [-E 0|1]"\
...
void *gpio_sample_thread(void *arg);

static int check_params(void)
{
    if (mode_set >  1 || ( mode != 0 && mode != 1)) {
        ALOGE("fetch mode error!\n");
        return -1;
    }

    if (keyup_event_set > 1 || (keyup_event != 0 && keyup_event != 1)) {
        ALOGE("keyup event error!\n");
        return -1;
    }
    if (repeat_event_set > 1 || repeat_interval_set > 1) {
        ALOGE("Only -g [-s -e] option can be set more than once!\n");
        return -1;
    }
    return 0;
}
static int ir_get_prot_status(char *prot)
{
    ...
    if (ret) {
        IR_Close();
        ALOGE("Fail to get protocol status!\n");
        return -1;
    }
...
static int ir_sample_set_prot_enable(char *prot, int enable)
{
    ...
    ALOGE("Try set enable status %d to protocol %s.\n", enable, prot);
    if (enable)
        ret = IR_EnableProtocol(prot);
    else
        ret = IR_DisableProtocol(prot);
    if (ret) {
        ALOGE("Fail to set enable status of %s\n",prot);
        IR_Close();
        return -1;
    }

    ALOGE("Check it out if we do have set succussfully!\n");
    ...
    while ((op = getopt(argc, argv, "m:u:U:r:R:T:E:g:ls:e:hD")) != -1) {
        switch (op) {
            case 'm' :
                mode = atoi(optarg);
                mode_set ++;
                ALOGE("key fetch mode set to :%d\n", mode);
                break;
            case 'u' :
                keyup_event = atoi(optarg);
                keyup_event_set ++;
                ALOGE("keyup event set to %d\n",keyup_event);
                break;
            case 'U' :
                keyup_timeout = strtoul(optarg, 0, 10);
                keyup_timeout_set ++;
                ALOGE("keyup timeout set to %d\n",keyup_timeout);
                break;
            case 'r' :
                repeat_event = atoi(optarg);
                repeat_event_set ++;
                ALOGE("repeat event set to %d\n",repeat_event);
                break;
            case 'R' :
                repeat_interval = strtoul(optarg, 0, 10);
                repeat_interval_set ++;
                ALOGE("repeat interval set to %d\n",repeat_interval);
                break;
            case 'T' :
                read_timeout = strtoul(optarg, 0, 10);
                ALOGE("read syscall timeout set to %d\n",read_timeout);
                break;
            case 'E':
                ir_enable = atoi(optarg);
                if (ir_enable != 0 && ir_enable != 1) {
                    ALOGE("-E parameter not correct, only 0 or 1 allowed!\n");
                    break;
                }
                ir_enable_set = 1;
                ALOGE("Set ir mode to %s state!\n", ir_enable ? "enable": "disable");
                break;
            case 'g' :
                ALOGE("try to get the enable status of %s\n",optarg);

                prot_status = ir_get_prot_status(optarg);
                if (prot_status >= 0)
                    ALOGE("The status of %s is :%s\n",
                            optarg,
            ...
            case 'l' :
                {
                    int i;
                    ALOGE("Available default protocols list:\n");
            ...
            case 'e' :
                ...
                ALOGE("protocol enable status will set to :%d\n",prot_enable);
    ...
    while (!g_thread_should_stop) {
        if (mode) {
            ...
            if (!ret) {
                ...
                ALOGE( "IR got value linux_key:%d; status:%d; infraredState:%d;...",
                ...
                if (android::InfraredService::infraredState() || (u32MouseMode && keycode_flag) ) {
                    ALOGE( "We are entering into MOUSE MODE!");
                    // Browser mode or not -- checking
                    if (!android::InfraredService::infraredState()) {
                        ALOGE( "We are currently in non-browser mode!");
                    ...
                    ALOGE( "Current g_mouse_move_step - %d!", g_mouse_move_step);
                ...
                if (debug_enable)
                    ALOGE("Received key: 0x%.08llx,linux_keycode =0x%x,%d,%d, %s,\tprotocol: %s.\n",
			...
			{
				ALOGE( "warn IR got value status:%d ",status);
				if(status == KEY_STATUS_DOWN)
				{
					ReportKeyEvent(linux_keycode,KPC_EV_KEY_RELEASE);
				}
...
static int PowerResumSet()
{
    ...
    if (HI_SUCCESS != ret)
    {
        ALOGE("HI_UNF_PMOC_Init err, 0x%08x !\n", ret);
        return ret;
    }

    memset(&wakeup, 0, sizeof(HI_UNF_PMOC_WKUP_S));
    memset(&devType, 0, sizeof(HI_UNF_PMOC_DEV_TYPE_S));
    property_get("persist.suspend.mode", tmp, "deep_restart");//customer.mk
...
#ifdef Hi3798CV200
    devType.kltype = (HI_UNF_KEYLED_TYPE_E) 2 ; //HI_UNF_KEYLED_TYPE_CT1642;
#endif

    ret = HI_UNF_PMOC_SetDevType(&devType);
    if (HI_SUCCESS != ret)
    {
        ALOGE("HI_UNF_PMOC_SetDevType err, 0x%08x !\n", ret);
        HI_UNF_PMOC_DeInit();
        return ret;
    }
...
#ifdef MCU
    wakeup.bGpioWakeUpEnable = (HI_BOOL)0x1;
    wakeup.u32GpioNo = 5 * 8 + 1;
#endif

    ret = HI_UNF_PMOC_SetWakeUpAttr(&wakeup);
    if (HI_SUCCESS != ret)
    {
        ALOGE("HI_UNF_PMOC_SetWakeUpAttr err, 0x%08x !\n", ret);
        HI_UNF_PMOC_DeInit();
        return ret;
    }

    ALOGE("PowerResumSet  ok.\n");
...
int main(int argc, char *argv[])
{
    ...
    ret = Hi_Vinput_Init();
    if (ret == -1){
        ALOGE("can't open hi_vinput,so we can't use ir ,please check vinput driver\n");
        exit(-1);
    }
    ...
    ret = GPIO_Open();
    if (ret) {
        perror("Fail to open gpio dev!");
        exit(-1);
    }
    if (!mode) {
       IR_SetCodeType(0);
        if (keyup_event_set) {
            ret = IR_EnableKeyUp((BOOL)keyup_event);
            if (ret) {
        ...
        if (keyup_timeout_set) {
            ALOGE("This version cannot support dynamicly change keyup timeout!\n");
        }

        if (repeat_event_set) {
            ret = IR_EnableRepKey((BOOL)repeat_event);
            if (ret) {
    ...
    ret = pthread_create(&gpio_thread, NULL, gpio_sample_thread, NULL);
    if (ret < 0) {
        perror("Failt to create gpio thread!");
        exit(-1);
    }
    pthread_join(thread, NULL);
    pthread_join(gpio_thread, NULL);
    ALOGE( "Ir sample thread exit! Bye!\n");
    /////////////////////////////////////////////////////////////
...
void *gpio_sample_thread(void *arg) {
    ...
    ALOGE("HISI_pmoc: gpio_sample_thread ALOGE ");
    ALOGE("HISI_pmoc: gpio_sample_thread ALOGE ");
		ALOGE("gongcm REset Gpio \n");
	ret = HI_UNF_PMOC_GetWakeUpAttr(&stAttr);
	standby_gpio = stAttr.u32GpioNo;
	reset_gpio  = 5*8+2;
	
	GPIO_SetDirBit(standby_gpio, TRUE);
	GPIO_SetDirBit(reset_gpio, TRUE);
    while (1) {
		bVal = 1;
		ret = GPIO_ReadBit(standby_gpio, &bVal);
		if (!ret) {

			linux_keycode = 0;

			if (!bVal){
				linux_keycode = KEY_POWER;
				ReportKeyEvent(linux_keycode,KPC_EV_KEY_PRESS);
				ReportKeyEvent(linux_keycode,KPC_EV_KEY_RELEASE);
			}

			if (debug_enable)
				ALOGE("standby gpio key was pressed.\n");
		}
		usleep(1000*30);
		
		bVal = 1;
		ret = GPIO_ReadBit(reset_gpio, &bVal);
		if (!ret) {

			linux_keycode = 0;

			if (!bVal){
				linux_keycode = 0x1c4;// KEY_RESET
				ReportKeyEvent(linux_keycode,KPC_EV_KEY_PRESS);
				ReportKeyEvent(linux_keycode,KPC_EV_KEY_RELEASE);
				ALOGE("resetgpio key was pressed.\n");
			}
