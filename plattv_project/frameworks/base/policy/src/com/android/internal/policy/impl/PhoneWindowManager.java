    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if(SystemProperties.get("ro.product.target").equals("shcmcc")){
            if (keyCode == KeyEvent.KEYCODE_BACK
              ...
              if (keyCode == KeyEvent.KEYCODE_RESET){
        	
        				if(down){
        				Intent clearMaster = new Intent("android.intent.action.MASTER_CLEAR");
        				mContext.sendBroadcast(clearMaster);	
        				Log.d(TAG," gongcm Reset android.intent.action.MASTER_CLEAR KEY_RESET ");
        				return 0;
        				}
        			}

				boolean isFactory = SystemProperties.get("ro.di.stb_factorytest_finish").equals("success");
				boolean isSecondLoading = SystemProperties.get("persist.sys.area.flag").equals("setted");
				
			if( isFactory && isSecondLoading){   
        if (keyCode == KeyEvent.KEYCODE_NOTIFICATION){
        	
        		if(down){
        				//Intent clearMaster = new Intent("android.intent.action.MASTER_CLEAR");
        				mContext.sendBroadcast(new Intent("cn.caratel.keyCode.Notification"));	
        			//	mContext.startActivity(new Intent("cn.caratel.tvapp.action.Notification"));
        				Log.d(TAG,"  gongcm start  cn.caratel.tvapp.action.Notification��������������������");
        				return 0;
        		}
        	}
         ...
         if (keyCode == KeyEvent.KEYCODE_CALL){
        		...
        		if(down){
        				    mContext.sendBroadcast(new Intent("cn.caratel.keyCode.Dial"));	
                		Log.d(TAG,"gongcm cn.caratel.keyCode.Dial send ");
                		return 0;
        		}
        	}
        	...
        	if (keyCode == KeyEvent.KEYCODE_ENDCALL){
        		...
        		if(down){
        				//Intent clearMaster = new Intent("android.intent.action.MASTER_CLEAR");
        				mContext.sendBroadcast( new Intent("cn.caratel.keyCode.Hangup"));	
                Log.d(TAG,"gongcm mContext.sendBroadcast new Intent cn.caratel.keyCode.Hangup");
                return 0;
        		}
        	}
        ...
        // timeout.
        if ((keyCode == KeyEvent.KEYCODE_HOME&& isFactory) && isSecondLoading) {

            // If we have released the home key, and didn't do anything else
            // while it was pressed, then it is time to go home!
						   if(down){
                     Intent intent = new Intent(Intent.ACTION_MAIN);
                     intent.setClassName("com.ipanel.join.homed.hunan", "com.ipanel.join.homed.chongqing.portal.PortalActivity2");
                     intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
                     intent.addCategory(Intent.CATEGORY_LAUNCHER);
                     mContext.startActivity(intent);
                     Log.d(TAG,"gongcm start Home !!!!!!!!!");
                     return -1;
                }
            if (!down) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_ENDCALL: {
                ...
                if (down) {
                  ...
                  Log.d(TAG,"gongcm Don't Go to sleep \n");//
                }
                break;
            }
