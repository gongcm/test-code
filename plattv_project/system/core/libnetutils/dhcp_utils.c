                    ...
                    const char *option125Info)
{
    ...
    if (property_get(HOSTNAME_PROP_NAME, prop_value, NULL) && (prop_value[0] != '\0')) {
        if(supportOption60 && supportOption125 && password != NULL) {//DHCP+ OPTION125
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-h %s --username --password -j --require 125 %s", DAEMON_NAME,
                     p2p_interface, prop_value, interface);
            ...
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-h %s -i -j --require 125 %s", DAEMON_NAME,
                     p2p_interface, prop_value, interface);
            property_set(VERDORID, login);
            property_set(VERDORSPEC, option125Info);
        } else if (supportOption60 && !supportOption125 && password != NULL) {//DHCP+
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-h %s --username --password %s", DAEMON_NAME,
                     p2p_interface, prop_value, interface);
            property_set(USERNAME, login);
            property_set(PASSWD, password);
        } else if (supportOption60 && !supportOption125 && password == NULL) {//OPTION60
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-h %s -i %s", DAEMON_NAME,
                     p2p_interface, prop_value, interface);
             property_set(VERDORID, login);
        } else if (!supportOption60 && supportOption125) {//OPTION125
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-h %s -j --require 125 %s", DAEMON_NAME,
                     p2p_interface, prop_value, interface);
            property_set(VERDORSPEC, option125Info);
        } else {
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-h %s %s", DAEMON_NAME,
                     p2p_interface, prop_value, interface);
        }
    }
    else {
        if(supportOption60 && password != NULL && supportOption125) {//DHCP+ OPTION125
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:--username --password -j --require 125 %s", DAEMON_NAME,
                     p2p_interface, interface);
            property_set(VERDORSPEC, option125Info);
            property_set(USERNAME, login);
            property_set(PASSWD, password);
        } else if (supportOption60 && password == NULL && supportOption125) {//OPTION60 OPTION125
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-i -j  --require 125 %s", DAEMON_NAME,
                     p2p_interface, interface);
            property_set(VERDORSPEC, option125Info);
            property_set(VERDORID, login);
        } else if (supportOption60 && password != NULL && !supportOption125) {//DHCP+
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:--username --password %s", DAEMON_NAME,
                     p2p_interface, interface);
            property_set(USERNAME, login);
            property_set(PASSWD, password);
        } else if (supportOption60 && password == NULL && !supportOption125) {//OPTION60
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-i %s", DAEMON_NAME,
                     p2p_interface, interface);
            property_set(VERDORID, login);
        } else if (!supportOption60 && supportOption125) {//OPTION125
            snprintf(daemon_cmd, sizeof(daemon_cmd), "%s_%s:-j --require 125 %s", DAEMON_NAME,
                     p2p_interface, interface);
            property_set(VERDORSPEC, option125Info);
        } else {
